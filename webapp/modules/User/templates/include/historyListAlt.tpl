...
{php}
    ...
    // 「もっと見る」のURLをアサイン。
    $action = ($this->get_template_vars('type') == 'comment') ? 'TimeLine' : 'FriendsHistory';
    $this->assign('moreUrl', Common::genContainerURL('User', $action, array(
...
{if $list.totalRows > 0}
  ...
  {foreach from=`$list.resultset` item="row"}
    ...
    {if $count > 1}{image_tag file='hr.gif'}{/if}<br />
  {/foreach}
