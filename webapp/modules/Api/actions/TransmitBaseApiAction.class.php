...
/**
 * FLASHからのloadVariablesを処理するアクションの基底クラス。
 */
abstract class TransmitBaseApiAction extends UserBaseAction {
    ...
    protected function respond($resData) {
        ...
        if(Common::getCarrier() != "android" && Common::getCarrier() != "iphone"){
            ...
            foreach($resData as &$value){
                $value = Common::adaptString($value, $purpose);
            }
        }

        $response = http_build_query($resData);
