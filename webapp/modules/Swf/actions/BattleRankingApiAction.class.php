 ...
 * ---------------------------------------------------------------------------------
 */
class BattleRankingApiAction extends ApiBaseAction {
    ...
    protected function doExecute($params) {
        ...
        foreach($list['resultset'] as &$record) {
            ...
            // 画像情報を取得。
            $spec = CharaImageUtil::getSpec($record['avatar']);
            $path = sprintf('%s.%s.gif', $spec, 'full');
            $record['avatar']['imageUrl'] = $path;
            ...
            //$prof = PlatformApi::queryProfile($record["user_id"], array('thumbnailUrl'));
            //$record["thumbnailUrl"] = $prof["thumbnailUrl"];
            //$record["thumbnailUrlSmall"] = $prof["thumbnailUrlSmall"];
            //$record["thumbnailUrlLarge"] = $prof["thumbnailUrlLarge"];
