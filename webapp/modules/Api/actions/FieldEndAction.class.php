 ...
 * クエスト結果を処理するアクション。
 */
class FieldEndAction extends SmfBaseAction {
    ...
    protected function doExecute($params) {
        ...
        //メニュー内クエストへのURL
        $array['urlOnQuest'] = Common::genContainerUrl('Api', 'Quest', array(), true);
        ...
        if(!$record)
            return array("result" => "error", "err_code"=>"no_sphere");
        ...
        if($nextId){
            ...
            $array['urlOnNext'] = Common::genContainerUrl('Api', 'QuestDrama', array("questId" => $nextId), true);       
        }else{
            $array['next'] = null;
        }
