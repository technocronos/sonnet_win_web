...
abstract class Controller extends MojaviObject
{
    ...
    public function forward ($moduleName, $actionName)
    {
        ...
        if (constant('MOD_' . strtoupper($moduleName) . '_ENABLED'))
        {
            ...
            // Mojavi標準では、フィルタより先にアクションを作成・初期化するようになっているが、
            // フィルタのほうを先に初期化するように変更する。
    ...
    protected function initialize ()
    {
		...
		$this->logger->setExitPriority(Logger::FATAL);
		$this->logger->setPriority(Logger::P_INFO);
		$layout = new PatternLayout(MO_LOG_PATTERN_LAYOUT);

		$fileAppender = new FileAppender($layout, MO_LOG_DIR . '/' . MO_LOG_FILENAME);
		$this->logger->removeAppender("stdout");
		$this->logger->setAppender("file", $fileAppender);
    ...
    public function setRenderMode ($mode)
    {
        ...
        if ($mode == View::RENDER_CLIENT || $mode == View::RENDER_VAR ||
            $mode == View::RENDER_NONE || $mode == View::RENDER_OUTPUT)
        {
