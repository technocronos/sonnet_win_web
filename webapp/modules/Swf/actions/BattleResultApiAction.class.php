...
class BattleResultApiAction extends ApiBaseAction {
    ...
    protected function doExecute($params) {
        ...
        if(!$battle  ||  $battle['true_status'] < Battle_LogService::SETTLE_BORDER) {
            return array('response'=> $battle ? $battle['true_status'] : -1);
        }
    ...
    private function setupResultView($battle) {
        ...
        if($battle['bias_user_id'] == $this->user_id) {
            ...
            foreach($battle['bias_result']['equip']['after'] as &$equip) {
                ...
                // 耐久値が警告値を下回っている場合...
                    ...
                    if($current  &&  $current['durable_count'] <= User_ItemService::USEFUL_WARN) {
                        ...
                        $equip['repaire_useto'] = ViewUtil::serializeBackto(
                            array('action'=>'Battle', 'repaireId'=>$equip['user_item_id'], 'firstscene' => 'result' , 'backto'=>null
                        ));

                        //アイテム使用ページからの戻るボタンの戻り先
                        $backtoLink = ViewUtil::serializeBackto(
                            array('module'=>'Swf', 'action'=>'Battle', 'firstscene' => 'result', 'battleId' => $_GET['battleId'], 'side' => $_GET['side'], 'backto'=>null
                        ));

                        //修理のリンク
                        $equip['urlOnRepaire'] = Common::genContainerUrl(
                            'User', 'Suggest', array('type' => 'repaire', 'targetId' => $equip['user_item_id'], 'useto' => $equip['repaire_useto'], 'backto' => $backtoLink), true
                        );
                ...
                //MAXレベル
                $maxLv = Service::create('Item_Level_Master')->getMaxLevel($equip['item_id']);
            ...
            if($battle['bias_result']['gain']['monster']) {
                $array['capture'] = Service::create('Monster_Master')->needRecord($battle['bias_result']['gain']['monster']);
                    ...
                    $array['capture']['image_url'] = $path;
            }
        }

        // 詳細データをビューにセット。
        $array['ready'] = $battle['bias_ready'];
        $array['result'] = $battle['bias_result'];

        //グレード昇格してるかどうか
        $before = $array['ready']['grade_id'];
        $after = $array['result']['character']['grade_id'];
        ...
        //フィールドに戻る場合
        $array['urlOnMain'] = Common::genContainerUrl(
            'Swf', 'Main', array(), true
        );

        //フィールドに戻る場合
        $array['urlOnQuest'] = Common::genContainerUrl(
            'Swf', 'Sphere', array('id'=>$battle['relate_id']), true
        );

        //対戦相手一覧へ戻る場合
        $array['urlOnList'] = Common::genContainerUrl(
            'Swf', 'Main', array('firstscene' => 'rival'), true
        );

        //対戦相手のページへの場合
        $array['urlOnMypage'] = Common::genContainerUrl(
            'Swf', 'Main', array('firstscene' => 'his_page', 'his_user_id'=>$array['battle']['rival_user_id']), true
        );
