 ...
 * 仮想通貨換金依頼を送信する
 */
class VcoinSendAction extends SmfBaseAction {
    ...
    protected function doExecute($params) {
        ...
        //これまでの課金額を得る。
        //以前の最終申請日時を得る
        $paymentdate = Service::create('Vcoin_Payment_Log')->getLatestApplyDate($this->user_id);

        //最終申請日時がある、または出金申請リセット制限開始日時以降である場合
        if($paymentdate != null && strtotime($paymentdate) > strtotime(BTC_APPLY_RESTRICT_DATE))
            $payment = Service::create('Payment_Log')->sumupUserPaymentWithDate($this->user_id, $paymentdate);
        else
            $payment = Service::create('Payment_Log')->sumupUserPayment($this->user_id);

        // 仮想通貨出金可能課金額
        if($payment[0]["sales"] < VCOIN_MINIMAM_PAYMENT){
            return array("result" => "short_payment", "short_payment" => VCOIN_MINIMAM_PAYMENT - $payment[0]["sales"]);
        }
