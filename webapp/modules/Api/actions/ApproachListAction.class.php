 ...
 * ---------------------------------------------------------------------------------
 */
class ApproachListAction extends SmfBaseAction {
...
//        );
        ...
        if($list["list"]["resultset"]){
            foreach($list["list"]["resultset"] as &$row){
                ...
                // 画像情報を取得。
                $spec1 = $this->getFormation($row["companion"]["chara"]);
    ...
    private function processOperation() {
        ...
        if($opCode == 'accept' || $opCode == 'reject' || $opCode == 'cancel') {
            ...
            return array('result'=>"ok" , "opCode" => $opCode, 'companion_id'=>$companionId);

        // 「確認済みにする」の場合。
        }else if($opCode == 'clear') {

            $apprSvc->confirmResult($this->user_id);
            return array('result'=>"ok" , "opCode" => $opCode);
        }else{
            return array('result'=>"error" , "opCode" => "");
        }
    }
}
