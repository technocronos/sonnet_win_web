...
<html>
  ...
  <body style="background-color:#000000; color:#FFFFFF; margin:0px; padding:0px;">
      <!--audioタグを使う場合、サウンドをタグで指定aaa-->
      {if count($use_audio_tag) > 0}
          {foreach from=`$use_audio_tag` item='audio_name'}
              <audio id="{$audio_name}" preload="auto">
                <!--iphoneはm4aでないとweb audio apiと併用する時に謎現象が・・audioタグのサウンドを出したらwebaudioの方の音が出なくなったりとてつもなく割れたりする-->
                {if $carrier == 'iphone'}
                    <source src="{sound_url file=`$web_audio_list.$audio_name` containar=0 ext='m4a'}" type="audio/mpeg">
                {else}
                    <source src="{sound_url file=`$web_audio_list.$audio_name` containar=0 ext='mp3'}" type="audio/mp3">
                {/if}
              </audio>
          {/foreach}
      {/if}
...
//alert(deviceheight);//960
        ...
        {literal}
          window.onload = function(event) {
              ...
              //ipad対応。16:9にする
              if(is_tablet == 'tablet' && carrier == "iphone")
                  devicewidth = (deviceheight * 9) / 16;

              //XperiaZ3tabletcompact等、タブレット対応。16:9にする
              if(is_tablet == 'tablet' && carrier == "android")
                  devicewidth = (deviceheight * 9) / 16;

              $("#out").css("height", deviceheight + "px");
              $("#out").html(eval("DramaSwfContentsHtml"));
