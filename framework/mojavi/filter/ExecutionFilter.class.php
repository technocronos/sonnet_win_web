    ...
    public function execute ($filterChain)
    {
        ...
        } else
        {
            ...
            if ($validated && $actionInstance->validate())
            {
                ...
                // ビュー名が返されていない場合は "Success" とする。
                if( is_null($viewName) )
                    $viewName = View::SUCCESS;
        ...
        if ($viewName != View::NONE)
        {
            ...
            if (!$controller->viewExists($moduleName, $viewName))
            {
                ...
                // デフォルトビューをチェック。
                $defaultViewName = $moduleName . 'Base';

                // デフォルトビューがあるならそれを使う。
                if ($controller->viewExists($moduleName, $defaultViewName))
                {
            ...
            // デフォルトテンプレート名をセットする。
            $viewInstance->setTemplate($originalViewName);
