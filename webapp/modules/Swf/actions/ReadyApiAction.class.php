 ...
 * 「対戦相手リスト」を処理するアクション。
 */
class ReadyApiAction extends ApiBaseAction {
    ...
    protected function doExecute($params) {
        ...
        if($avatar['sally_sphere']){
            $this->redirect('User', 'FieldReopen');
        }
        ...
        if( !Service::create('Field_Master')->getRecord($_GET['questId']) )
            $this->redirect('User', 'Static', array('id'=>'UnderConstruct'));
    ...
    private function setOpenComment() {
        ...
        if($_GET['questId'] == 11002) {
            ...
            if(!$cleared) {
                $comments[] = "フィールドクエストに\nでかけるときは、この画面で\n持ってくアイテム選ぶのだ";
                $comments[] = "持ち出したいアイテムをタップして\n持ち出すのだ";
                $comments[] = "持ち出せるアイテムは種類によって\n制限あるのだ\n気を付けるのだ";
                $comments[] = "クエスト終わったら使ってないやつは\n回収されるのだ\nだから遠慮なく持ち出すのだ";
                $comments[] = "じゃ、やってみるのだ";
            }
        }
    ...
    private function processPost() {
        ...
        if( !$questObj->isExecutable() )
            $this->redirect('Swf', 'Main', array("firstscene" => "quest"));
    ...
    private function processMove() {
        ...
        if($arrivalEvent)
            $this->redirect('Swf', 'QuestDrama', array('questId'=>$arrivalEvent));
