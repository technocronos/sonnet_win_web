...
abstract class View extends MojaviObject
{
    ...
    /**
     * Render the presentation to a file.
     *
     * @since 3.0.0
     */
    const RENDER_OUTPUT = 5;
    ...
    /**
     * ビューの処理結果を出力するためのディレクトリ名を取得する
     * ※ディレクトリが存在しない場合は、ディレクトリを作成する
     *
    ...
    public function getOutputDirectory ()
    {
		...
		if (!is_dir($this->outputDirectory)) {
			if (!mkdir($this->outputDirectory, 0755, true)) {
				throw new ApplicationException("Can not make CDROM Dir! dir=" . $this->outputDirectory);
			}
		}
...
#         }
    ...
    /**
     * ビューの処理結果を出力するためのディレクトリ名をセットする
     *
	 * @param string $directory 出力ディレクトリ名
	 * @return void
     *
	 * @author T.Wakuda (wakuda@withit.co.jp)
	 * @since 2007/01/06
    ...
    /**
	 * ビューの出力結果を出力するためのファイル名をセットする
	 * （絶対パスを受け取った場合、出力ディレクトリのセットはいらない）
	 *
    ...
    public function setOutputFilename ($filename)
    {
        ...
        if (Toolkit::isPathAbsolute($filename))
        {
            ...
            $this->outputDirectory = dirname($filename);
            $this->outputFilename  = basename($filename);
